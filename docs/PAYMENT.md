...
RelayDeck has a built-in payment system that enables user self-service top-up without deploying a separate payment service.
...
> **EasyPay provider selection**: RelayDeck does not recommend, endorse, or
> receive compensation from any provider. Choose a provider compatible with the
> EasyPay protocol based on your jurisdiction, funding channel, settlement
> currency, and compliance requirements.
>
> - **Domestic channel / CNY settlement** — verify that the provider supports
>   the payment methods, settlement account type, and transaction limits you
>   require before enabling it.
> - **International channel / foreign-currency settlement** — verify supported
>   payment methods, settlement routes, fees, withdrawal conditions, and any
>   required business verification before enabling it.
>
...
If you previously used an external payment service as an external payment system, you can migrate to the built-in payment system:

### Key Differences

| Aspect | External payment service | Built-in Payment |
|--------|-----------|-----------------|
| Deployment | Separate service (Next.js + PostgreSQL) | Built into RelayDeck, no extra deployment |
| Payment Methods | EasyPay, Alipay, WeChat, Stripe | Same |
| Configuration | Environment variables + separate admin UI | Unified in RelayDeck admin dashboard |
| Top-up Integration | Via Admin API callback | Internal processing, more reliable |
| Subscription Plans | Supported | Not yet (planned) |
| Order Management | Separate admin interface | Integrated in RelayDeck admin dashboard |
