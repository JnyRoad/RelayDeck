...
func EvaluatePluginCompatibility(manifest PluginManifest, host PluginHostInfo) PluginCompatibility {
	result := PluginCompatibility{
		CurrentRelayDeck:     host.Version,
		RequiredRelayDeck:    manifest.Requires.RelayDeck,
		RecommendedRelayDeck: manifest.Requires.RecommendedRelayDeckVersion,
	...
	if manifest.Requires.PluginProtocol != pluginv1.ProtocolVersion ||
		...
		result.Message = "插件协议版本与当前 RelayDeck 不兼容"
		return result
	}
	if !matchesSemverRange(host.Version, manifest.Requires.RelayDeck) {
		result.Status = "incompatible"
		result.Message = fmt.Sprintf("当前 RelayDeck %s 不满足插件要求 %s", host.Version, manifest.Requires.RelayDeck)
		return result
	}
	result.Compatible = true
	for _, tested := range manifest.Requires.TestedRelayDeckVersions {
	...
	if result.Tested {
		...
		result.Message = "当前 RelayDeck 版本已由插件声明测试"
	} else {
		result.Status = "untested"
		result.Message = "版本范围兼容，但插件未声明已测试当前 RelayDeck 版本"
	}
	return result
}
