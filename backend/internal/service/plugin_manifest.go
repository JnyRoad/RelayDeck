...
type PluginRequirements struct {
	RelayDeck                   string   `json:"relaydeck"`
	RecommendedRelayDeckVersion string   `json:"recommended_relaydeck_version,omitempty"`
	TestedRelayDeckVersions     []string `json:"tested_relaydeck_versions,omitempty"`
...
type PluginCompatibility struct {
	...
	CurrentRelayDeck     string `json:"current_relaydeck_version"`
	RequiredRelayDeck    string `json:"required_relaydeck_version"`
	RecommendedRelayDeck string `json:"recommended_relaydeck_version"`
	PluginProtocol       int    `json:"plugin_protocol"`
	TransportAPI         int    `json:"transport_api"`
	UIBridge             int    `json:"ui_bridge"`
}
...
func (m PluginManifest) Validate() error {
	...
	if strings.TrimSpace(m.Requires.RelayDeck) == "" {
		return errors.New("插件必须声明 requires.relaydeck")
	}
