...
// 探测资格：/v1/relaydeck/billing 是 key 级端点，全部
...
func TestUpstreamBillingProbeGrokAccountPersistsSnapshot(t *testing.T) {
	...
	require.Equal(t, "https://relay.example/v1/relaydeck/billing", upstream.lastReq.URL.String())
...
// /v1/relaydeck/billing：不发请求，直接落 unsupported。
...
// 必无 /v1/relaydeck/billing，不发请求直接 unsupported。
...
func TestUpstreamBillingProbeOpenAIDefaultBaseURLPreserved(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, "https://api.openai.com/v1/relaydeck/billing", upstream.lastReq.URL.String())
