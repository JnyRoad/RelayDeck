...
func TestResolveLogFilePath_WithDataDir(t *testing.T) {
	t.Setenv("DATA_DIR", "/tmp/relaydeck-data")
	got := resolveLogFilePath("")
	want := filepath.Join("/tmp/relaydeck-data", "logs", "relaydeck.log")
	if got != want {
...
func TestBuildFileCore_InvalidPathFallback(t *testing.T) {
	...
	opts.Output.FilePath = filepath.Join(os.DevNull, "logs", "relaydeck.log")
