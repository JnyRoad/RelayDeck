...
func (p *darwinProvider) Generate(ctx context.Context) (string, error) {
	...
	command.Env = []string{
		"PATH=/usr/bin:/bin",
		"RELAYDECK_DEVICECHECK_MODULE=" + modulePath,
		"RELAYDECK_ATTESTATION_BUNDLE_ID=" + bundleID,
		"RELAYDECK_ATTESTATION_SIGNALS=" + string(signalsJSON),
	}
...
const addon = require(process.env.RELAYDECK_DEVICECHECK_MODULE);
const signals = JSON.parse(process.env.RELAYDECK_ATTESTATION_SIGNALS);
const bundleID = process.env.RELAYDECK_ATTESTATION_BUNDLE_ID;
