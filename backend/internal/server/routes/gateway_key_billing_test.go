...
import (
	...
	"github.com/JnyRoad/RelayDeck/internal/web"
...
func TestGatewayRoutesKeyBillingInfoPathIsRegistered(t *testing.T) {
	...
	for _, route := range router.Routes() {
		if route.Method == http.MethodGet && route.Path == "/v1/relaydeck/billing" {
			return
		}
	}

	t.Fatal("GET /v1/relaydeck/billing should be registered")
}
...
func TestGatewayRoutesKeyBillingInfoEndToEnd(t *testing.T) {
	t.Run("missing credentials", func(t *testing.T) {
		...
		router.ServeHTTP(w, httptest.NewRequest(http.MethodGet, "/v1/relaydeck/billing", nil))
	...
	t.Run("standard mode", func(t *testing.T) {
		...
		req := httptest.NewRequest(http.MethodGet, "/v1/relaydeck/billing", nil)
		...
		require.Equal(t, "relaydeck.key_billing", body["object"])
