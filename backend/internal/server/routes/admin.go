...
func registerUserManagementRoutes(admin *gin.RouterGroup, h *handler.Handlers) {
	users := admin.Group("/users")
	{
		...
		users.PUT("/:id/api-keys/:key_id", h.Admin.User.UpdateUserAPIKey)
		users.DELETE("/:id/api-keys/:key_id", h.Admin.User.DeleteUserAPIKey)
		users.GET("/:id/api-keys/available-groups", h.Admin.User.GetUserAPIKeyAvailableGroups)
		users.GET("/:id/api-keys/group-rates", h.Admin.User.GetUserAPIKeyGroupRates)
...
func registerModelTraceRoutes(admin *gin.RouterGroup, h *handler.Handlers, stepUpAuth middleware.StepUpAuthMiddleware) {
	traces := admin.Group("/model-traces")
	{
		...
		traces.GET("/:traceID/conversation", gin.HandlerFunc(stepUpAuth), h.Admin.ModelTrace.Conversation)
		traces.POST("/:traceID/access-events", gin.HandlerFunc(stepUpAuth), h.Admin.ModelTrace.RecordAccessEvent)
