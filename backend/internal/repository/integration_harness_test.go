...
func TestMain(m *testing.M) {
	...
	pgContainer, err := tcpostgres.Run(
		ctx,
		postgresImage,
		tcpostgres.WithDatabase("relaydeck_test"),
...
//
// RELAYDECK_TEST_POSTGRES_IMAGE overrides the PostgreSQL image so the suite can be
...
//
//	RELAYDECK_TEST_POSTGRES_IMAGE=postgres:15-alpine go test -tags integration ./internal/repository/
func selectDockerImage(ctx context.Context, preferred string) string {
	if override := strings.TrimSpace(os.Getenv("RELAYDECK_TEST_POSTGRES_IMAGE")); override != "" &&
