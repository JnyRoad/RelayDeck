...
func TestPluginsMigrationKeepsAccountSchemaUnchanged(t *testing.T) {
	...
	require.Contains(t, sql, "CREATE TABLE IF NOT EXISTS relaydeck_plugin_installations")
	require.Contains(t, sql, "CREATE TABLE IF NOT EXISTS relaydeck_plugin_bindings")
	require.Contains(t, sql, "config_encrypted TEXT NOT NULL DEFAULT ''")
	require.Contains(t, sql, "REFERENCES relaydeck_plugin_installations(id)")
	require.Contains(t, sql, "CREATE UNIQUE INDEX IF NOT EXISTS idx_relaydeck_plugin_bindings_one_enabled_scope")
...
func TestPluginArtifactMigrationSupportsExistingInstallations(t *testing.T) {
	...
	sql := strings.Join(strings.Fields(string(content)), " ")
	require.Contains(t, sql, "ALTER TABLE relaydeck_plugin_installations")
