...
type DialOptions struct {
	...
	// CompressionClientMaxWindowBits offers client_max_window_bits without a value.
	// When the server selects a value, client messages are constrained to that
	// negotiated DEFLATE window. It has no effect when CompressionMode is disabled.
	CompressionClientMaxWindowBits bool
...
func dial(ctx context.Context, urls string, opts *DialOptions, rand io.Reader) (_ *Conn, _ *http.Response, err error) {
	...
	if opts.CompressionMode != CompressionDisabled {
		...
		copts.clientMaxWindowBitsOffered = opts.CompressionClientMaxWindowBits
	}
...
func verifyServerExtensions(copts *compressionOptions, h http.Header) (*compressionOptions, error) {
	...
	if copts.clientMaxWindowBitsOffered {
		// RFC 7692 permits an omitted response value. Codex uses its maximum
		// supported window in that case.
	...
	for _, p := range ext.params {
		...
		if strings.HasPrefix(p, "client_max_window_bits=") {
			...
			windowBits, err := strconv.Atoi(strings.TrimPrefix(p, "client_max_window_bits="))
			if err != nil || windowBits < 9 || windowBits > 15 {
				return nil, fmt.Errorf("unsupported permessage-deflate parameter: %q", p)
			}
