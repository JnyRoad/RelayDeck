...
type msgWriter struct {
	...
	flateWriter     messageFlateWriter
	flateWindowBits int
}
...
// ensureFlate initializes the writer selected by the negotiated client window.
func (mw *msgWriter) ensureFlate() error {
	...
	if mw.flateWriter == nil {
		mw.flateWindowBits = mw.clientWindowBits()
		writer, err := getFlateWriter(mw.trimWriter, mw.flateWindowBits)
		if err != nil {
			return err
		}
		mw.flateWriter = writer
	}
	mw.flate = true
	return nil
}

// clientWindowBits returns the client-side window selected during extension negotiation.
func (mw *msgWriter) clientWindowBits() int {
	if mw.c.client && mw.c.copts != nil {
...
func (mw *msgWriter) putFlateWriter() {
	if mw.flateWriter != nil {
		putFlateWriter(mw.flateWriter, mw.flateWindowBits)
		mw.flateWriter = nil
		mw.flateWindowBits = 0
	}
}
