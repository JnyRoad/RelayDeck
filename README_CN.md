...
**面向多上游模型的自托管 AI API 网关：安全路由、统一密钥、配额与可观测性。**
...
RelayDeck 是面向团队和自托管场景的 AI API 网关，将多个上游服务接入统一控制面。它提供账号路由、API Key 分发、配额感知计费、并发控制和可观测性，并向客户端提供一致的 API。
...
通过 Nginx 反向代理 RelayDeck（或 CRS 服务）并搭配 Codex CLI 使用时，需要在 Nginx 配置的 `http` 块中添加：
...
Apple 芯片 Mac 在 macOS 26 上可使用 Apple `container` 1.1.0 或更高版本运行完整的 RelayDeck、PostgreSQL 和 Redis：
...
- `security.trust_forwarded_ip_for_api_key_acl` 控制旧版原始转发头接管（为升级兼容默认开启）；关闭后严格使用 `server.trusted_proxies`，其中只应填写直接连接 RelayDeck 的精确代理 CIDR
...
1. **推荐——让向导自动生成 `config.yaml`：** 跳过上面的第 5 步（不要执行 `cp`）。直接运行 `./relaydeck`，访问 `http://localhost:8080`，向导会引导你完成数据库、Redis 和管理员账号配置，并自动写出 `config.yaml`。

2. **如果你已经创建了 `config.yaml`：** 首次启动前先把它临时移走以触发向导，完成后再恢复：
   ```bash
   mv config.yaml config.yaml.bak
   ./relaydeck        # 向导在 http://localhost:8080 启动，并生成新的 config.yaml
   # 向导完成后 Ctrl+C 停服，再恢复你的配置：
   mv config.yaml.bak config.yaml
   ./relaydeck        # 重启进入正常模式，用刚创建的管理员登录
   ```
...
websocat -H="Sec-WebSocket-Protocol: relaydeck-admin, jwt.<ADMIN_TOKEN>" ws://localhost:8080/api/v1/admin/ops/ws/qps
```
...
RelayDeck 支持 [Antigravity](https://antigravity.so/) 账户，授权后可通过专用端点访问 Claude 和 Gemini 模型。
