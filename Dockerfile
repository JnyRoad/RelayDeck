...
# =============================================================================
# RelayDeck Multi-Stage Dockerfile
# =============================================================================
...
RUN --mount=type=cache,id=relaydeck-pnpm-store,target=/root/.local/share/pnpm/store \
...
RUN --mount=type=cache,id=relaydeck-gomod,target=/go/pkg/mod \
    --mount=type=cache,id=relaydeck-gobuild,target=/root/.cache/go-build \
...
LABEL description="RelayDeck - AI API Gateway Platform"
LABEL org.opencontainers.image.source="https://github.com/JnyRoad/RelayDeck"
...
RUN addgroup -g 1000 relaydeck && \
    adduser -u 1000 -G relaydeck -s /bin/sh -D relaydeck

# Set working directory
WORKDIR /app

# Copy binary/resources with ownership to avoid extra full-layer chown copy
COPY --from=backend-builder --chown=relaydeck:relaydeck /app/relaydeck /app/relaydeck
COPY --from=backend-builder --chown=relaydeck:relaydeck /app/backend/resources /app/resources

# Create data directory
RUN mkdir -p /app/data && chown relaydeck:relaydeck /app/data

# Copy entrypoint script (fixes volume permissions then drops to relaydeck)
...
# Run the application (entrypoint fixes /app/data ownership then execs as relaydeck)
