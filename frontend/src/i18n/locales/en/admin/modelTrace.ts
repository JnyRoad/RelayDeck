export default {
  modelTrace: {
    ...
    detail: {
      title: 'Complete call chain', trace: 'Current trace', linked: 'Linked by {source}', unlinked: 'No verifiable conversation link was found; only this call is shown.', views: 'Call detail views', chat: 'Conversation replay', rawChain: 'Raw chain', currentTurn: 'Current call', user: 'User input', model: 'Model output', attempt: 'Upstream attempt #{number}', loadBody: 'Load body', loadMoreBody: 'Load more body', loadOlder: 'Load earlier turns', loadNewer: 'Load newer turns', loadingBody: 'Loading body…', payloadMetadata: 'Status: {status} · Original: {original} B · Stored: {stored} B · SHA-256: {hash}', contentUnavailable: 'Body is unavailable ({status}). Unstored, unreadable, and undecryptable content is never displayed.',
    },
