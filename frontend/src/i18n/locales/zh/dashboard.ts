export default {
  ...
  keys: {
    ...
    useKeyModal: {
      ...
      grok: {
        description:
          '配置 Grok CLI、Claude Code、Codex 或 OpenCode，让请求通过当前 RelayDeck Grok 分组发送。文本模型走 Responses；图片/视频使用 Imagine 模型 ID 与媒体端点。',
        claudeDescription: '配置 Claude Code，让 Messages API 请求通过当前 RelayDeck Grok 分组发送。',
        codexDescription: '配置 Codex，让 Responses API 请求通过当前 RelayDeck Grok 分组发送。',
        configTomlHint:
        ...
        codexConfigTomlHint:
          'Codex 官方：wire_api 仅支持 "responses"；优先 env_key，勿与 experimental_bearer_token 混用；非 OpenAI 网关默认 supports_websockets = false（RelayDeck 仍可接客户端 WS 并桥接到 HTTP/SSE）。合并前备份 ~/.codex/config.toml。',
        note:
        ...
        codexNote:
          '导出 RELAYDECK_API_KEY，将 config.toml 保存到 ~/.codex（可用 mkdir -p ~/.codex）。优先 env_key，勿提交密钥。',
        codexNoteWindows:
          '设置 $env:RELAYDECK_API_KEY，将 config.toml 保存到 %USERPROFILE%\\.codex。优先 env_key，勿提交密钥。'
      },
      deepseek: {
        ...
        codexNote: '启动 Codex 前先导出 RELAYDECK_API_KEY。下载的目录只包含模型元数据，不包含 API Key。'
      },
      composite: {
        description: '通过当前 Composite 路由分组配置受支持的客户端。',
        codexDescription: '使用 API Key 和当前 Composite 分组的完整模型目录配置 Codex。',
        codexConfigTomlHint: '下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。',
        codexNote: '启动 Codex 前先导出 RELAYDECK_API_KEY；分组会根据目录中选中的模型路由请求。'
      },
      routedCodex: {
        description: '使用当前路由分组的完整模型目录配置 Codex。',
        configTomlHint: '下载下方模型目录，将两个文件保存到 Codex 配置目录后重启 Codex。',
        note: '启动 Codex 前先导出 RELAYDECK_API_KEY。下载的目录只包含模型元数据，不包含 API Key。'
      },
      codexModelCatalog: {
        title: 'Codex 模型目录',
