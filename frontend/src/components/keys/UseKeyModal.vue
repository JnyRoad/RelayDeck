...
export XAI_API_KEY="${apiKey}"`
  ...
  // Shape follows Grok Build user guide (~/.grok/docs + custom-models) and production-ready RelayDeck setups.
  ...
  const configContent = `# Grok Build CLI → RelayDeck Grok group (API key auth).
# Docs: ~/.grok/docs/user-guide/05-configuration.md + 11-custom-models.md
# Verify after save: grok inspect
#
# IMPORTANT: api_backend must be "responses" for RelayDeck Grok (POST /v1/responses).
...
# Prefer API key when using a custom gateway (matches RelayDeck).
...
description = "Grok 4.5 via RelayDeck (Responses)"
...
# Imagine tools: model IDs go to RelayDeck media endpoints (not the text [model.*] catalog).
...
function generateGrokCodexFiles(baseUrl: string, apiKey: string): FileConfig[] {
  ...
  switch (shell) {
    case 'cmd':
      envPath = 'Command Prompt'
      envContent = `set RELAYDECK_API_KEY=${apiKey}`
      break
    case 'powershell':
    case 'windows':
      envPath = 'PowerShell'
      envContent = `$env:RELAYDECK_API_KEY="${apiKey}"`
      break
    default:
      envPath = 'Terminal'
      envContent = `export RELAYDECK_API_KEY="${apiKey}"`
  }

  const configContent = `# Codex CLI → RelayDeck Grok group
...
# Grok/RelayDeck path is HTTP/SSE; disable WS (Codex may otherwise try WebSocket first)
...
function generateRoutedCodexFiles(
  ...
  const envContent = isWindows
    ? `$env:RELAYDECK_API_KEY="${apiKey}"`
    : `export RELAYDECK_API_KEY="${apiKey}"`

  const configContent = `# Codex CLI -> RelayDeck ${label} group
model_provider = "relaydeck"
model = "${model}"
review_model = "${model}"
disable_response_storage = true
...
function generateOpenCodeConfig(platform: string, baseUrl: string, apiKey: string, pathLabel?: string): FileConfig {
  ...
  if (platform === 'gemini') {
    ...
    // Custom provider pointing at RelayDeck OpenAI-compatible Responses/Chat endpoints.
    provider[platform].npm = '@ai-sdk/openai-compatible'
    provider[platform].name = 'Grok via RelayDeck'
