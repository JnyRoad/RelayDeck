...
describe('UseKeyModal', () => {
  ...
  it('renders Grok Build and OpenCode setup for Grok groups', async () => {
    ...
    expect(parsed.provider.grok.name).toBe('Grok via RelayDeck')
  ...
  it('renders Codex custom provider setup through the Grok Responses gateway', async () => {
    ...
    let codeBlocks = wrapper.findAll('pre code').map((code) => code.text())
    const configToml = codeBlocks.find((content) => content.includes('[model_providers.relaydeck]'))
    expect(configToml).toBeDefined()
    expect(configToml).toContain('model_provider = "relaydeck"')
    expect(configToml).toContain('model = "grok-4.5"')
    expect(configToml).toContain('base_url = "https://example.com/v1"')
    expect(configToml).toContain('env_key = "RELAYDECK_API_KEY"')
  ...
  it('offers a downloadable Codex catalog for Composite API keys', async () => {
    ...
    const unixConfig = wrapper.findAll('pre code')
      .map((code) => code.text())
      .find((content) => content.includes('[model_providers.relaydeck]'))
    expect(unixConfig).toContain('model_catalog_json = "~/.codex/codex-models.json"')
    expect(unixConfig).toContain('env_key = "RELAYDECK_API_KEY"')
