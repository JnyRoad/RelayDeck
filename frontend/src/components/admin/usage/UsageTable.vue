        ...
        <template #cell-request_id="{ row }">
          <div v-if="row.request_id" class="flex max-w-[160px] items-center gap-1.5">
            ...
            <button
              ...
              :class="copiedRequestId?.type === 'request' && copiedRequestId.value === row.request_id ? 'text-green-500 hover:text-green-500' : ''"
              :title="copiedRequestId?.type === 'request' && copiedRequestId.value === row.request_id ? t('keys.copied') : t('keys.copyToClipboard')"
              @click="copyRequestId(row.request_id)"
            >
              <Icon :name="copiedRequestId?.type === 'request' && copiedRequestId.value === row.request_id ? 'check' : 'copy'" size="sm" class="h-3.5 w-3.5" />
            </button>
          </div>
          <span v-else class="text-sm text-gray-400 dark:text-gray-500">-</span>
        </template>
        ...
        <template #cell-upstream_request_id="{ row }">
          <div v-if="row.upstream_request_id" class="flex max-w-[160px] items-center gap-1.5">
            ...
            <button
              type="button"
              class="shrink-0 rounded p-0.5 text-gray-400 transition-colors hover:bg-gray-100 hover:text-gray-600 dark:hover:bg-dark-700 dark:hover:text-gray-300"
              :class="copiedRequestId?.type === 'upstream' && copiedRequestId.value === row.upstream_request_id ? 'text-green-500 hover:text-green-500' : ''"
              :title="copiedRequestId?.type === 'upstream' && copiedRequestId.value === row.upstream_request_id ? t('keys.copied') : t('keys.copyToClipboard')"
              @click="copyUpstreamRequestId(row.upstream_request_id)"
            >
              <Icon :name="copiedRequestId?.type === 'upstream' && copiedRequestId.value === row.upstream_request_id ? 'check' : 'copy'" size="sm" class="h-3.5 w-3.5" />
            </button>
          </div>
          <span v-else class="text-sm text-gray-400 dark:text-gray-500">-</span>
        </template>
...
type CopiedRequestIdType = 'request' | 'upstream'
interface CopiedRequestId {
  type: CopiedRequestIdType
  value: string
}
const copiedRequestId = ref<CopiedRequestId | null>(null)
...
const copyIdentifier = async (value: string, type: CopiedRequestIdType, copiedMessage: string) => {
  try {
    await navigator.clipboard.writeText(value)
    copiedRequestId.value = { type, value }
    appStore.showSuccess(copiedMessage)
    window.setTimeout(() => {
      if (copiedRequestId.value?.type === type && copiedRequestId.value.value === value) {
...
const copyRequestId = (requestId: string) => copyIdentifier(requestId, 'request', t('admin.usage.requestIdCopied'))
const copyUpstreamRequestId = (upstreamRequestId: string) =>
  copyIdentifier(upstreamRequestId, 'upstream', t('admin.usage.upstreamRequestIdCopied'))
