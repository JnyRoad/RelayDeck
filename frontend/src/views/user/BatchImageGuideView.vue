...
const PREVIEW_CACHE_DB_NAME = 'relaydeck-batch-image-preview-cache'
  ...
  if (!key) return
	  ...
	  try {
        ...
        items: parsedItems.value,
	      ...
	      `relaydeck-ui-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
	    )
...
async function retryFailedJob(job: BatchImageJobRow | BatchImageJob) {
  ...
  try {
    ...
    const retryJob = await submitBatchImageJob(
      ...
      `relaydeck-ui-retry-${job.id}-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`,
    )
