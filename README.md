...
<img src="assets/logo.svg" alt="RelayDeck Logo" width="128" />
...
**A self-hosted AI API gateway for secure, observable routing across multiple providers.**
...
RelayDeck is a self-hosted AI API gateway for teams that need one reliable control plane for multiple upstream providers. It combines account routing, API-key distribution, quota-aware billing, concurrency control, and observability behind a consistent client-facing API.
...
When using Nginx as a reverse proxy for RelayDeck (or CRS) with Codex CLI, add the following to the `http` block in your Nginx configuration:
...
3. Install binary to `/opt/relaydeck`
...
# Uninstall
curl -sSL https://raw.githubusercontent.com/JnyRoad/RelayDeck/main/deploy/install.sh | sudo bash -s -- uninstall -y
```
...
mkdir -p relaydeck-deploy && cd relaydeck-deploy
...
cd ..
tar czf relaydeck-complete.tar.gz relaydeck-deploy/

# Transfer to new server
scp relaydeck-complete.tar.gz user@new-server:/path/
...
Apple-silicon Macs running macOS 26 can run the full RelayDeck, PostgreSQL, and Redis stack with Apple `container` 1.1.0 or newer:
...
go build -tags embed -ldflags="-X main.Version=${VERSION}" -o relaydeck ./cmd/server
...
- `security.trust_forwarded_ip_for_api_key_acl` enables legacy raw forwarded-header takeover (enabled by default for upgrade compatibility); disable it to enforce `server.trusted_proxies`, which should contain only the exact proxy CIDRs that connect directly to RelayDeck
...
1. **Recommended — let the wizard generate `config.yaml`:** Skip step 5 (do not run the `cp`). Start `./relaydeck` directly; the setup wizard at `http://localhost:8080` walks you through database, Redis, and admin account setup, then writes `config.yaml` for you.
...
2. **If you already created `config.yaml`:** Temporarily move it aside so the wizard can trigger on first run, then restore it afterwards:
   ...
   ./relaydeck        # wizard runs at http://localhost:8080 and writes a fresh config.yaml
   # stop the server (Ctrl+C) once the wizard completes, then restore your config:
   mv config.yaml.bak config.yaml
   ./relaydeck        # restart in normal mode and log in with the admin you just created
   ```
...
RelayDeck supports both Grok subscription accounts through xAI OAuth and standard xAI API-key accounts. Both account types forward OpenAI-compatible Responses traffic to xAI.
...
1. In the RelayDeck admin dashboard, add either a `grok` OAuth account and complete xAI authorization, or add a Grok API-key account.
2. Create a Grok group, attach the account to it, then create a RelayDeck API key assigned to that group.
...
[model."grok"]
model = "grok-4.5"
base_url = "https://your-relaydeck.example.com/v1"
name = "Grok 4.5"
api_key = "sk-your-relaydeck-key"
api_backend = "responses"
context_window = 1000000
supports_backend_search = true
```

Back up an existing `config.toml` before merging the entry. The file contains a RelayDeck API key, so keep it private and restrict its permissions where supported. Verify the effective configuration and make a smoke request:

```bash
grok inspect
grok -p "Reply with relaydeck-ok" -m grok
```

The `base_url` above is the public RelayDeck URL ending in `/v1`, not `api.x.ai` or the internal xAI OAuth proxy URL.

### Usage And Quota Display

xAI quota is passive. RelayDeck does not invent subscription quota values; it records whitelisted xAI rate-limit headers from successful or rate-limited upstream responses when xAI sends them. Before the first usable upstream response, the dashboard shows quota as unknown and still displays local RelayDeck usage stats.
...
RelayDeck supports [Antigravity](https://antigravity.so/) accounts. After authorization, dedicated endpoints are available for Claude and Gemini models.
...
<a href="https://star-history.dera.page/#JnyRoad/RelayDeck&Date">
 <picture>
   <source media="(prefers-color-scheme: dark)" srcset="https://star-history.dera.page/svg?repos=JnyRoad/RelayDeck&type=Date&theme=dark" />
   <source media="(prefers-color-scheme: light)" srcset="https://star-history.dera.page/svg?repos=JnyRoad/RelayDeck&type=Date" />
   <img alt="Star History Chart" src="https://star-history.dera.page/svg?repos=JnyRoad/RelayDeck&type=Date" />
 </picture>
</a>
