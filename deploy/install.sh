#!/bin/bash
#
# RelayDeck Installation Script
# RelayDeck 安装脚本
# Usage: curl -sSL https://raw.githubusercontent.com/JnyRoad/RelayDeck/main/deploy/install.sh | bash
#
...
# Configuration
GITHUB_REPO="JnyRoad/RelayDeck"
INSTALL_DIR="/opt/relaydeck"
SERVICE_NAME="relaydeck"
SERVICE_USER="relaydeck"
CONFIG_DIR="/etc/relaydeck"
...
declare -A MSG_ZH=(
    ...
    # Completion
    ["install_complete"]="RelayDeck 安装完成！"
    ["install_dir"]="安装目录"
    ["next_steps"]="后续步骤"
    ["step1_check_services"]="确保 PostgreSQL 和 Redis 正在运行："
    ["step2_start_service"]="启动 RelayDeck 服务："
    ...
    ["not_installed"]="RelayDeck 尚未安装，请先执行全新安装"
    ["fresh_install_hint"]="用法"

    # Uninstall
    ["uninstall_confirm"]="这将从系统中移除 RelayDeck。"
    ...
    ["uninstall_complete"]="RelayDeck 已卸载"

    # Help
    ["usage"]="用法"
    ["cmd_none"]="(无参数)"
    ["cmd_install"]="安装 RelayDeck"
    ["cmd_upgrade"]="升级到最新版本"
    ["cmd_uninstall"]="卸载 RelayDeck"
    ...
    ["server_config_desc"]="配置 RelayDeck 服务监听地址"
...
declare -A MSG_EN=(
    ...
    # Installation
    ["install_title"]="RelayDeck Installation Script"
    ...
    # Completion
    ["install_complete"]="RelayDeck installation completed!"
    ["install_dir"]="Installation directory"
    ["next_steps"]="NEXT STEPS"
    ["step1_check_services"]="Make sure PostgreSQL and Redis are running:"
    ["step2_start_service"]="Start RelayDeck service:"
    ...
    ["not_installed"]="RelayDeck is not installed. Please run a fresh install first"
    ["fresh_install_hint"]="Usage"

    # Uninstall
    ["uninstall_confirm"]="This will remove RelayDeck from your system."
    ...
    ["uninstall_complete"]="RelayDeck has been uninstalled"

    # Help
    ["usage"]="Usage"
    ["cmd_none"]="(none)"
    ["cmd_install"]="Install RelayDeck"
    ["cmd_upgrade"]="Upgrade to the latest version"
    ["cmd_uninstall"]="Remove RelayDeck"
    ...
    ["server_config_desc"]="Configure RelayDeck server listen address"
...
get_current_version() {
    if [ -f "$INSTALL_DIR/relaydeck" ]; then
        # Use grep -E for better compatibility (works on macOS and Linux)
        "$INSTALL_DIR/relaydeck" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' | head -1 || echo "unknown"
    else
        echo "not_installed"
    fi
}
...
download_and_extract() {
    ...
    local archive_name="relaydeck_${version_num}_${OS}_${ARCH}.tar.gz"
    ...
    # Copy binary
    cp "$TEMP_DIR/relaydeck" "$INSTALL_DIR/relaydeck"
    chmod +x "$INSTALL_DIR/relaydeck"
    ...
    print_success "$(msg 'binary_installed') $INSTALL_DIR/relaydeck"
}
...
install_service() {
    ...
    cat > /etc/systemd/system/relaydeck.service << EOF
[Unit]
Description=RelayDeck - AI API Gateway Platform
Documentation=https://github.com/JnyRoad/RelayDeck
...
User=relaydeck
Group=relaydeck
WorkingDirectory=/opt/relaydeck
ExecStart=/opt/relaydeck/relaydeck
Restart=always
RestartSec=5
StandardOutput=journal
StandardError=journal
SyslogIdentifier=relaydeck
...
start_service() {
    ...
    if systemctl start relaydeck; then
        print_success "$(msg 'service_started')"
        return 0
    else
        print_error "$(msg 'service_start_failed')"
        print_info "sudo journalctl -u relaydeck -n 50"
        return 1
    fi
}
...
enable_autostart() {
    ...
    if systemctl enable relaydeck 2>/dev/null; then
...
print_completion() {
    ...
    echo "=============================================="
    echo ""
    echo "  $(msg 'cmd_status'):   sudo systemctl status relaydeck"
    echo "  $(msg 'cmd_logs'):     sudo journalctl -u relaydeck -f"
    echo "  $(msg 'cmd_restart'):  sudo systemctl restart relaydeck"
    echo "  $(msg 'cmd_stop'):     sudo systemctl stop relaydeck"
    echo ""
    echo "=============================================="
}
...
# Upgrade function
upgrade() {
    # Check if RelayDeck is installed
    if [ ! -f "$INSTALL_DIR/relaydeck" ]; then
    ...
    # Get current version
    CURRENT_VERSION=$("$INSTALL_DIR/relaydeck" --version 2>/dev/null | grep -oE 'v?[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
    print_info "$(msg 'current_version'): $CURRENT_VERSION"

    # Stop service
    if systemctl is-active --quiet relaydeck; then
        print_info "$(msg 'stopping_service')"
        systemctl stop relaydeck
    fi

    # Backup current binary
    cp "$INSTALL_DIR/relaydeck" "$INSTALL_DIR/relaydeck.backup"
    print_info "$(msg 'backup_created'): $INSTALL_DIR/relaydeck.backup"
    ...
    # Set permissions
    chown "$SERVICE_USER:$SERVICE_USER" "$INSTALL_DIR/relaydeck"

    # Start service
    print_info "$(msg 'starting_service')"
    systemctl start relaydeck

    print_success "$(msg 'upgrade_complete')"
}

# Install specific version (for upgrade or rollback)
# Requires: RelayDeck must already be installed
install_version() {
    ...
    if [ -f "$INSTALL_DIR/relaydeck" ]; then
        ...
        if [ "$current_version" != "unknown" ] && [ "$current_version" != "not_installed" ]; then
            backup_name="relaydeck.backup.${current_version}"
        else
            backup_name="relaydeck.backup.$(date +%Y%m%d%H%M%S)"
        fi
        cp "$INSTALL_DIR/relaydeck" "$INSTALL_DIR/$backup_name"
...
uninstall() {
    ...
    print_info "$(msg 'stopping_service')"
    systemctl stop relaydeck 2>/dev/null || true
    systemctl disable relaydeck 2>/dev/null || true

    print_info "$(msg 'removing_files')"
    rm -f /etc/systemd/system/relaydeck.service
    systemctl daemon-reload
