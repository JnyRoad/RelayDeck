...
ENV_FILE="${RELAYDECK_ENV_FILE:-${SCRIPT_DIR}/.env}"

STACK_LABEL_KEY="org.relaydeck.stack"
STACK_LABEL_VALUE="apple-container"
NETWORK_NAME="relaydeck-apple"
APP_CONTAINER="relaydeck-apple"
POSTGRES_CONTAINER="relaydeck-apple-postgres"
REDIS_CONTAINER="relaydeck-apple-redis"
APP_VOLUME="relaydeck-apple-data"
POSTGRES_VOLUME="relaydeck-apple-postgres-data"
REDIS_VOLUME="relaydeck-apple-redis-data"
PLATFORM="linux/arm64"

TEMP_DIR=""
LOCK_DIR="${TMPDIR:-/tmp}/relaydeck-apple-container.lock"
LOCK_ACQUIRED=false
...
Commands:
  ...
  up [--recreate]       Create and start the complete RelayDeck stack
...
Environment:
  RELAYDECK_ENV_FILE      Path to the deployment env file (default: deploy/.env)
EOF
}
...
acquire_lock() {
    if ! mkdir "${LOCK_DIR}" 2>/dev/null; then
        ...
        if [[ "${owner_pid}" =~ ^[0-9]+$ ]] && ! kill -0 "${owner_pid}" 2>/dev/null; then
            ...
            die "Another RelayDeck Apple container operation is already running."
        fi
    fi
    printf '%s\n' "$$" >"${LOCK_DIR}/pid"
    LOCK_ACQUIRED=true
...
cmd_init() {
    ...
    info "Review the file, then run: RELAYDECK_ENV_FILE='${ENV_FILE}' ${SCRIPT_DIR}/apple-container.sh up"
}
...
prepare_environment() {
    ...
    APP_IMAGE="$(read_env_value APPLE_CONTAINER_RELAYDECK_IMAGE jnyroad/relaydeck:latest)"
    ...
    POSTGRES_USER="$(read_env_value POSTGRES_USER relaydeck)"
    POSTGRES_PASSWORD="$(read_env_value POSTGRES_PASSWORD)"
    POSTGRES_DB="$(read_env_value POSTGRES_DB relaydeck)"
    ...
    TEMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/relaydeck-apple.XXXXXX")"
...
create_app_container() {
    info "Creating RelayDeck container..."
    container create \
        --name "${APP_CONTAINER}" \
        --label "${STACK_LABEL_KEY}=${STACK_LABEL_VALUE}" \
        --network "${NETWORK_NAME}" \
        ...
        --entrypoint /bin/sh \
        "${APP_IMAGE}" \
        -c 'set -e; mkdir -p "$DATA_DIR"; chown -R relaydeck:relaydeck "$DATA_DIR"; exec su-exec relaydeck /app/relaydeck' \
        >/dev/null
}
...
start_app() {
    ...
    if ! wait_for_probe "RelayDeck" 180 probe_app; then
        show_failure_logs "${APP_CONTAINER}"
        die "RelayDeck did not become ready."
    fi
    if ! wait_for_probe "RelayDeck host port" 15 probe_host_app; then
...
cmd_up() {
    ...
    info "RelayDeck is available at http://${ACCESS_HOST}:${HOST_PORT}"
}
...
cmd_down() {
    ...
    info "RelayDeck stack stopped; persistent volumes were preserved."
}
...
cmd_logs() {
    ...
    case "${service}" in
        app|relaydeck) container_name="${APP_CONTAINER}" ;;
...
confirm_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        printf 'Delete the RelayDeck stack and all persistent data? [y/N] '
    else
        printf 'Delete the RelayDeck containers and network, preserving volumes? [y/N] '
    fi
    read -r answer
    [[ "${answer}" == "y" || "${answer}" == "Y" ]]
}
...
cmd_destroy() {
    ...
    if [[ "${include_volumes}" == true ]]; then
        ...
        info "RelayDeck stack and persistent data deleted."
    else
        info "RelayDeck stack deleted; persistent volumes were preserved."
    fi
}
