...
RelayDeck can run as a native three-service stack with Apple's `container` CLI. This workflow runs the published RelayDeck, PostgreSQL, and Redis OCI images without Docker Desktop or a Docker-compatible daemon.
...
# Creates volumes/network/containers, waits for dependencies, and starts RelayDeck.
...
# Restart PostgreSQL, Redis, and RelayDeck in dependency order.
...
The script uses `deploy/.env`, the same source file used by Docker Compose. Export `RELAYDECK_ENV_FILE` to use another file for every command in the current shell:

```bash
export RELAYDECK_ENV_FILE=/absolute/path/to/relaydeck.env
./apple-container.sh init
./apple-container.sh up
```
...
|---|---|
| Application and gateway variables | Passed to RelayDeck from `.env` |
...
| `REDIS_PASSWORD` | Applied to Redis and RelayDeck |
...
The script creates only resources carrying the `org.relaydeck.stack=apple-container` label:

| Type | Names |
|---|---|
| Containers | `relaydeck-apple`, `relaydeck-apple-postgres`, `relaydeck-apple-redis` |
| Network | `relaydeck-apple` |
| Volumes | `relaydeck-apple-data`, `relaydeck-apple-postgres-data`, `relaydeck-apple-redis-data` |

The PostgreSQL volume is mounted at `/var/lib/postgresql`, retaining PostgreSQL 18's default child data directory. RelayDeck and Redis also store data in child directories below their Apple volume mount points. This is required because Apple named volumes do not have Docker's copy-up and mount-point ownership behavior.

## Networking

Apple `container` 1.1 does not provide Compose-style network-scoped service aliases. After PostgreSQL and Redis start, the script reads their current private-network IPv4 addresses from `container inspect`, injects those addresses into a newly created application container, and then starts RelayDeck. The script does not modify `~/.config/container/config.toml` or the macOS host resolver.

All three services attach only to the private `relaydeck-apple` network. Only the application publishes a host port; database and Redis ports remain unpublished.

The application container is intentionally recreated by every `up` and `restart` operation because dependency VM addresses can change after they stop. Application data remains in `relaydeck-apple-data`.
...
container exec relaydeck-apple sh -c \
  'PGPASSWORD="$DATABASE_PASSWORD" pg_dump -h "$DATABASE_HOST" -U "$DATABASE_USER" "$DATABASE_DBNAME"' \
  > backups/relaydeck.sql

# Application configuration and local files.
container exec relaydeck-apple sh -c 'tar -C "$DATA_DIR" -czf - .' \
  > backups/relaydeck-data.tar.gz
...
container run --rm --name relaydeck-apple-data-restore \
  ...
  "$RELAYDECK_IMAGE" \
  -c 'rm -rf /restore/data && mkdir -p /restore/data && tar -xzf /backup/relaydeck-data.tar.gz -C /restore/data'

# Restore the logical database while the application is absent.
container start relaydeck-apple-postgres
until container exec relaydeck-apple-postgres sh -c 'pg_isready -U "$POSTGRES_USER" -d "$POSTGRES_DB"'; do sleep 1; done
container copy backups/relaydeck.sql relaydeck-apple-postgres:/tmp/relaydeck.sql
container exec relaydeck-apple-postgres sh -c '
  export PGPASSWORD="$POSTGRES_PASSWORD"
  dropdb -h 127.0.0.1 -U "$POSTGRES_USER" --if-exists --force "$POSTGRES_DB"
  createdb -h 127.0.0.1 -U "$POSTGRES_USER" "$POSTGRES_DB"
  psql -h 127.0.0.1 -U "$POSTGRES_USER" -d "$POSTGRES_DB" -v ON_ERROR_STOP=1 -f /tmp/relaydeck.sql
  rm /tmp/relaydeck.sql
'
...
- The script targets native `linux/arm64` images. The normal RelayDeck release publishes an arm64 variant.
